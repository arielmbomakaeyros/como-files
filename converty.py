...
# # Generate dataPerXnecAndXra
# def generate_data_per_xnec_and_xra(xnec_id, xra_id, timestamp):
#     return {
#         "timestamp": timestamp,
...
def generate_dataset():
    base_time = datetime(2025, 2, 9, 0, 30)  # Start from 2024-10-01T00:30:00Z
    data_per_xnec = []
    data_per_xra = []
    data_per_xnec_and_xra = []

    # Generate dataPerXnec and dataPerXra
    for i in range(5):  # Assuming 5 items for dataPerXnec and dataPerXra
        timestamp = generate_timestamp(base_time, i)
        data_per_xnec.append(generate_data_per_xnec(timestamp))
        data_per_xra.append(generate_data_per_xra(timestamp))

    # Generate dataPerXnecAndXra
    xnec_ids = [xnec["xnecId"] for xnec in data_per_xnec]
    xra_ids = [xra["xraId"] for xra in data_per_xra]

    for i in range(len(data_per_xnec) + len(data_per_xra)):
        timestamp = generate_timestamp(base_time, i)
        xnec_id = random.choice(xnec_ids)
        xra_id = random.choice(xra_ids)
        data_per_xnec_and_xra.append(generate_data_per_xnec_and_xra(xnec_id, xra_id, timestamp))

    return {
        "dataPerXnec": data_per_xnec,
        "dataPerXra": data_per_xra,
        "dataPerXnecAndXra": data_per_xnec_and_xra
    }
# ===============================================
...
def generate_sample_data_final(schema, full_schema, start_date, offset_days):
    ...
    # # Generate dataPerXnec, dataPerXra, and dataPerXnecAndXra
    ...
    # # Add the generated data to the final object
