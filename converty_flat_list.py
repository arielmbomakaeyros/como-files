...
def generate_sample_data_final(schema, start_date, offset_days, tso_eic_to_area_code):
    ...
    sample_data = {
        ...
        "contingencyGroupList": generate_contingency_group_list(schema),
        "contingencyList": generate_contingency_list(schema),
        "contingencyListId": str(uuid.uuid4()),  # Use native UUID
...
def generate_contingency_group_list(schema):
    if random.choice([True, False]):
        return None
    return [{
        "equipmentList": generate_equipment_list(schema),
        "groupCode": str(uuid.uuid4())  # Use native UUID
...
def generate_contingency_list(schema):
    ...
    return [{
        ...
        "equipmentOperatorCode": str(uuid.uuid4()),  # Use native UUID
        "groupCode": str(uuid.uuid4()),  # Use native UUID
        "mrid": str(uuid.uuid4()),  # Use native UUID
        "mustStudy": random.choice([True, False]),
        "name": f"Contingency {i+1}" if random.choice([True, False]) else None,
        "probability": round(random.uniform(0, 100), 2) if random.choice([True, False]) else None
    } for i in range(random.randint(1, 5))]

# Helper function to generate an equipment list
def generate_equipment_list(schema):
    if random.choice([True, False]):
        return None
    return [{
        "areaCode": str(uuid.uuid4()),  # Use native UUID
        "description": f"Sample equipment description {i+1}" if random.choice([True, False]) else None,
        "equipmentMrid": str(uuid.uuid4()),  # Use native UUID
        "equipmentType": random.choice(["bus", "busbar", "gen", "line", "load", "shuntCompensator", "staticVarCompensator", "switch", "transformer"]),
        "mrid": str(uuid.uuid4()),  # Use native UUID
        "name": f"Equipment {i+1}" if random.choice([True, False]) else None
...
# Example usage
baseFileName = "CO_Flat_List_Schema_fixed"
